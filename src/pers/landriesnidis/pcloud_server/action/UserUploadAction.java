...
public class UserUploadAction extends BaseServletAction {

	public UserUploadAction() throws IOException {
		super("UserUpload");
	}
		...
		try {
			...
			if(!SqlQueryHelper.CheckFileExists(filemd5)){
				...
				receiveFile(request, new OnFileReceiveListener() {
					@Override
					public void onFinished(String md5, long size) throws SQLException {
						SqlQueryHelper.FileUpload(token, md5, size);
					}
				});
			}
...
//					System.out.println(file.getPath());
					...
					listener.onFinished(MD5.getMD5CodeByFile(file), file.length());
					break;//一次请求只接受一个文件
				}
			}
	}
	...
	interface OnFileReceiveListener{
		void onFinished(String md5,long size) throws SQLException;
	}
